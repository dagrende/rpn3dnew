...
export default {
  ...
  union(state) {
    if (state.stack.length < 2) {
      ...
      state.stack.splice(0, 2, state.stack[1].union(state.stack[0]));
    }
  },
  subtract(state) {
    if (state.stack.length < 2) {
      ...
      state.stack.splice(0, 2, state.stack[1].subtract(state.stack[0]));
    }
  },
  intersect(state) {
    if (state.stack.length < 2) {
      ...
      state.stack.splice(0, 2, state.stack[0].intersect(state.stack[1]));
    }
  }
}
